...
function ensure(){
  ...
  // ── Sağ üst Mondaine saat (yalnızca saat) ──
  ...
  if(!window.__scScroll){
    window.addEventListener('scroll',function(){if(window.__scProg)window.__scProg();document.body.classList.toggle('sc-scrolled',(window.scrollY||window.pageYOffset)>150);},{passive:true});
...
function perNav(){
  ...
  // ── 4c: Sol araç çubuğuna Genişlet + Fokus tuşları (hilal/güneş yanına) ──
  var dm=document.querySelector('.darkmode');
  var bar=dm?(dm.closest('.flex-component')||dm.parentElement):null;
  if(bar&&!bar.querySelector('.sc-toolbtn')){
    ...
    bar.appendChild(be); bar.appendChild(bf);
  }
